...
/** Aplicação pública anônima de um diagnóstico do CATÁLOGO (motor dinâmico). */
export function PublicDiagnosticForm({ slug }: { slug: string }) {
  const [info, setInfo] = useState<PublicDiagnosticInfo | null>(null);
    ...
    return (
      <div className={s.wrap}>
        <div className={s.card}>
          <p className={s.state}>Carregando diagnóstico…</p>
        </div>
      </div>
    );
  ...
  return (
    <div className={s.wrap}>
      <div className={s.card}>
        <Brand />
        <span className={s.pill}>{info?.instrumentName}</span>
        <h1 className={s.title}>{info?.tenantName}</h1>
        <p className={s.sub}>
          São {questions.length} afirmações — responda de 1 (discordo totalmente) a 5 (concordo totalmente).
        </p>
        <p className={s.note}>
          <strong>Anônimo.</strong> Não pedimos seu nome e não guardamos nada que te identifique. Os
          resultados são vistos pela empresa apenas de forma <strong>agregada</strong>.
        </p>
