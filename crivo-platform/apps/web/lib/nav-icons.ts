// Ícones de traço do menu do portal do cliente — os MESMOS do protótipo Lovable
// (Portal do Cliente › app-sidebar.tsx, lucide-react 0.575.0, licença ISC).
// Vêm como marcação SVG, e não como componente React, porque a sidebar ainda é
// HTML gerado da nav.config (renderNavHtml) e injetado no shell legado.
// Chave = nome do ícone no lucide (LineChart do protótipo é 'chart-line',
// FileCheck2 é 'file-check-corner' e FileSignature é 'file-pen-line': na 0.575
// os nomes antigos são só apelidos destes).
...
/** SVG completo do ícone: traço na cor do texto (currentColor), decorativo. */
export function navIconSvg(name: NavIconName): string {
  return (
